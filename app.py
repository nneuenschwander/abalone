...
### __Table 1: Data Dictionary as Stated from UCI ML Repo__
...
From the Pair Plot and Heat map, we can see that there is high correlation between the different weight features. This can cause multicolinearity in a model that we may need to account for. We can also see that the length and diameter are highly correlated.''')
st.markdown('''### Weights
...
The objective is to predict whether or not an observation will have greater than 5 rings or not. Currently, rings is a continuous variable that we will need to convert into a binary variable that will end up being the response variable of our modeling efforts.
`Figure {figures}` shows the distribution of records that have rings greater than 5. As we can see, 95% of the data set has a more than 5 rings, we will have to consider different sampling methods in order to account for this.''')
